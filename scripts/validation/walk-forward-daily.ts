#!/usr/bin/env npx tsx
/**
 * scripts/validation/walk-forward-daily.ts — Test B
 *
 * Stratégie originale 7 filtres, mais sur données Daily (1d) — 5 ans.
 * Sur Daily :
 *   EMA20 = 20 jours (1 mois), EMA50 = 50 jours (2.5 mois), EMA200 = 200 jours (10 mois)
 *   Donchian24 = 24 jours (~1 mois), ADX classique pour tendance journalière
 *
 * Plus de barres par fenêtre → intervalles de confiance plus solides.
 *
 * Usage : npx tsx scripts/validation/walk-forward-daily.ts
 * Pré-requis : avoir lancé fetch-daily.ts
 */
...
const LOOKBACK   = 400;  // >> EMA200 sur Daily
const MAX_HOLD   = 20;   // 20 barres Daily ≈ 1 mois max
...
async function main() {
  const files = fs.readdirSync(DATA_DIR).filter(f => f.endsWith('_1d.json')).sort();
  if (!files.length) { console.error('❌ Fichiers Daily manquants — lancer fetch-daily.ts d\'abord.'); process.exit(1); }
  ...
  console.log(`\n📊 Test B — Stratégie 7 filtres | Daily | ${datasets.length} actifs | ${WINDOWS_N} fenêtres (~${barsPerWin} barres/fenêtre)\n`);
  ...
  console.log('\n════════════════════════════════════════ Résultats Test B (7 filtres Daily) ══');
  ...
  console.log(`\n  Verdict Test B : ${verdict}\n`);

  fs.writeFileSync(path.join(DATA_DIR, '_test_b_7filter_daily.json'), JSON.stringify({ generatedAt: Date.now(), strategy: '7-filter-daily', results, aggregate: agg, verdict }, null, 2));
}
